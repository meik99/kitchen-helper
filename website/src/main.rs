...
fn App() -> Element {
    let login_state = use_signal(|| state::login::LoginState::new());
    let mut navbar_state = use_signal(|| false);

    use_future(move || async move {
        loop {
            sleep(Duration::from_secs(300)).await;

            let current_login_state = login_state.read();

            if (current_login_state.is_valid() && current_login_state.should_refresh()) {
                info!("refresh token")
            } else {
                info!("no refresh needed")
            }
        }
    });

    rsx! {
        if login_state.read().is_valid() {
            div { class: "pf-v6-c-page",
                    ...
                    div { class: "pf-v6-c-masthead__main",
                        span { class: "pf-v6-c-masthead__toggle",
                            button {
                                ...
                                onclick: move |_| {
                                    let open = *navbar_state.read();
                                    *navbar_state.write() = !open;
                                },
                        ...
                        div { class: "pf-v6-c-masthead__brand",
                            a { href: "#", class: "pf-v6-c-masthead__logo",
                                svg { "viewBox": "0 0 679 158", height: "37px",
                                    title { "PF-HorizontalLogo-Color" }
                                    defs {
                                        linearGradient {
                                            "x1": "68%",
                                            "y2": "100%",
                ...
                div {
                    ...
                    class: if !*navbar_state.read() { "pf-m-collapsed" },
                    div { class: "pf-v6-c-page__sidebar-body", "Navigation" }
                }
                div { tabindex: "-1", class: "pf-v6-c-page__main-container",
                    main { tabindex: "-1", class: "pf-v6-c-page__main",
                        Router::<Route> {}
                    }
                }
            }
